    ...
    <div className="space-y-6">
      <div className="card">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="btn-outline py-2 px-4 text-sm">
              {uploading ? 'Uploading…' : '📷 Upload Photo'}
            </button>
            <p className="text-xs text-midGray mt-1">JPG or PNG · Max 2MB</p>
          </div>
        </div>
      ...
      <div className="card space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="input" type="tel" value={form.phone} onChange={e => setForm(f => ({...f, phone: e.target.value}))} placeholder="Optional" />
          </div>
          <div>
          ...
          {SPECIALTIES_OPTIONS.map(s => (
            <button key={s} onClick={() => toggleSpecialty(s)}
              className={`px-3 py-1.5 rounded-full text-sm font-semibold border transition-colors ${
                form.specialties.includes(s) ? 'bg-forest text-white border-forest' : 'border-tan text-midGray hover:border-sage'
              }`}>
              {s}
            </button>
          ))}
          ...
          <div className="space-y-2">
            {assignedClients.map(c => (
              <div key={c.id} className="flex items-center justify-between p-3 bg-cream rounded-lg">
                ...
                <a href={`/admin/clients/${c.id}`} className="text-forest text-xs font-semibold hover:underline">Manage →</a>
              </div>
            ))}
          </div>
        </div>
