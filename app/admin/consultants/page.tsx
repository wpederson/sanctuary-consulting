          ...
          return (
            <div key={con.id} className={`card ${!con.active ? 'opacity-60' : ''}`}>
              <div className="flex items-center gap-3 mb-4">
                {con.photo_url ? (
                  <img src={con.photo_url} alt={con.name} className="w-14 h-14 rounded-xl object-cover flex-shrink-0" />
                ) : (
              ...
              <div className="flex gap-2">
                <Link href={`/admin/consultants/${con.id}`} className="btn-primary py-1.5 px-3 text-xs">✏️ Edit</Link>
              </div>
            </div>
          )
        })}
